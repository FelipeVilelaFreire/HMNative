...
import { styles } from './EditActivityModal.styles';
...
export default function EditActivityModal({ visible, activity, onClose, onSave }: EditActivityModalProps) {
  ...
  // Preenche o form quando a activity muda
  useEffect(() => {
    if (activity) {
      ...
      // Tenta encontrar o espaço baseado no nome (mock)
      const space = spaces.find(s => s.name === activity.location?.address?.split(',')[0]);
  ...
  const handleSave = () => {
    if (!activity || !name || !category || !price) return;
  ...
  const isFormValid = name && category && price;
